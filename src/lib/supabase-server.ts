...
import { getLiveDataBackend } from './data-backend';
...
export function createAuthServiceRoleSupabaseClient(): ServiceRoleSupabaseClientLike {
...
export function createServiceRoleSupabaseClient(): ServiceRoleSupabaseClientLike {
  if (getLiveDataBackend() === 'local') {
    return createMockServiceRoleClient();
  }

  return createAuthServiceRoleSupabaseClient();
}
